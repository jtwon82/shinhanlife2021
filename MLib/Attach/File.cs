...
namespace MLib.Attach
{
    public class File
    {
        public string _saveFileName = string.Empty;
        public string _saveFilePath = string.Empty;
        public string _ext = string.Empty;
        public long _size = 0;
        ...
        /// 업로드 제한
        /// </summary>
        /// <param name="type">파일 ContentType</param>
        /// <returns>bool : 파일타입 체크</returns>
        public bool LimitType(string type)
        {
            string limit = "application/xml;text/html;application/javascript;text/css;";
            if (!Check.IsIn(limit, type))
            {
                return true;
            }
            else
        ...
        /// 속성 셋팅
        /// </summary>
        /// <param name="info">FileInfo 파일 객체</param>
        public void Properties(FileInfo info)
        {
            if (info.Exists)
            {
                _result = true;
                _saveFileName = info.Name;
                _saveFilePath = info.FullName;
                _ext = info.Extension.Replace(".", "");
                _size = info.Length;
                _type = MimeType(info.Extension);
            }
        }
        ...
        /// 파일경로 중복 제거
        /// </summary>
        /// <param name="path">원본파일 전체경로</param>
        /// <returns>string 중복 제거된 파일 전체경로</returns>
        public string Distinct(string path, bool overwrite)
        {
            ...
            if (info.Exists)
            {
                if (!overwrite)
                {
                    ...
                    do
                    {
                        path = Path.Combine(root, Guid.NewGuid().ToString("N") + ext);
        ...
        /// <returns>string 파일 속성(minetype)</returns>
